...
export default function Faq(){
    return(
        <>
            ...
            {/* tablits */}
            <div data-aos='zoom-in' className='lg:hidden hidden absolute z-5 top-0 left-0 w-full h-300 md:flex justify-center items-center'>
                <div className=" container h-2/3 w-full text-white flex-col gap-10">
                    <h2 className="text-3xl pe-10">- FAQ</h2>
                    <div className=" h-full bg-red-900/20 backdrop-blur-sm p-15">
                        <Accordion defaultValue={["shipping"]} className="size-full">
                            <div data-aos='fade-right' className="w-full h-full p-10 grid grid-rows-16">
                                    {faqData.map((q , i) => {
                                        return(
                                            <AccordionItem className=' h-[12.5%]' key={i} value={q.value}>
                                                <AccordionTrigger className=' cursor-pointer '>{q.question}</AccordionTrigger>
            ...
            {/* phones */}
            <div data-aos='zoom-in' className='md:hidden absolute z-5 p-10 top-0 left-0 w-full flex justify-center items-center'>
                <div className=" container w-full text-white flex-col gap-10 py-30">
                    <h2 className="text-3xl">- FAQ</h2>
                    <div className=" h-full bg-red-900/20 backdrop-blur-sm p-5">
                        <Accordion defaultValue={["shipping"]} className="size-full">
